...
namespace Tignino {
    ...
    class Alumno {
       ...
       public static function listarObjetos() {
        ...
        while (!feof($ar)) {
            $linea = fgets($ar);
            $alumno_info = explode("-", $linea);
            $alumno_info = array_map('trim', $alumno_info);
    
            // Crea un objeto Alumno y agrega al array
            $alumnos[] = new Alumno(
                (int)$alumno_info[0],
                $alumno_info[1],
                $alumno_info[2],
    ...
    public static function listarTabla() {
        ...
        $alumnos = self::listarObjetos();
    
        foreach ($alumnos as $alumno) {
            $tabla .= '<tr>';
            $tabla .= '<td>' . $alumno->legajo . '</td>';
            $tabla .= '<td>' . $alumno->apellido . '</td>';
            $tabla .= '<td>' . $alumno->nombre . '</td>';
            $tabla .= '<td><img src="' . $alumno->foto . '" alt="' . $alumno->apellido . '"></td>';
            $tabla .= '</tr>';
        }
